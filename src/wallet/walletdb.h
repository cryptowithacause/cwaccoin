...
public:
    ...
    bool WriteZCWCSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZCWCSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZCWCSeed_deprecated(uint256& seed);
    bool EraseZCWCSeed();
    bool EraseZCWCSeed_deprecated();

    bool WriteZCWCCount(const uint32_t& nCount);
    bool ReadZCWCCount(uint32_t& nCount);
