...
CzCWCTracker::CzCWCTracker(std::string strWalletFile)
{
...
CzCWCTracker::~CzCWCTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzCWCTracker::Init()
{
...
bool CzCWCTracker::Archive(CMintMeta& meta)
{
...
bool CzCWCTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzCWCTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzCWCTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzCWCTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzCWCTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzCWCTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzCWCTracker::GetSerialHashes()
{
...
CAmount CzCWCTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzCWCTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzCWCTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzCWCTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzCWCTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzCWCTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzCWCTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzCWCTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzCWCTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzCWCTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzCWCTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzCWCWallet* zCWCWallet)
{
    bool iszCWCWalletInitialized = (NULL != zCWCWallet);
    CMintMeta meta;
    ...
    if (! iszCWCWalletInitialized)
        zCWCWallet = new CzCWCWallet(strWalletFile);
    meta.isSeedCorrect = zCWCWallet->CheckSeed(dMint);
    if (! iszCWCWalletInitialized)
        delete zCWCWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzCWCTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzCWCTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzCWCTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzCWCTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzCWCTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzCWCTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzCWCWallet* zCWCWallet = new CzCWCWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zCWCWallet);
        }
        delete zCWCWallet;
        LogPrint("zero", "%s: added %d dzcwc from DB\n", __func__, listDeterministicDB.size());
    }
