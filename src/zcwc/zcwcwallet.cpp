...
CzCWCWallet::CzCWCWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zcwc seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZCWCSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZCWCSeed_deprecated()) {
                    LogPrintf("%s: Updated zCWC seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zcwc seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zcwc wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzCWCWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZCWCCount(nCountLastUsed);
    else if (!walletdb.ReadZCWCCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzCWCWallet::Lock()
{
    seedMaster = 0;
}

void CzCWCWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzCWCWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZCWC(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzCWCWallet::LoadMintPoolFromDB()
{
...
void CzCWCWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzCWCWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzCWCWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zcwcTracker->HasPubcoinHash(pMint.first)) {
...
bool CzCWCWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZCWC(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zcwcTracker which also adds to database
    pwalletMain->zcwcTracker->Add(dMint, true);
...
void CzCWCWallet::SeedToZCWC(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzCWCWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzCWCWallet::GenerateDeterministicZCWC(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzCWCWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzCWCWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzCWCWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
