...
// The zCWC block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zCWC is a hash of the serial
...
{
    //Create an output returning the zCWC that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZCWCOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zCWC output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zCWC", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZCWCOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zCWC output", __func__);
...
{
    CzCWCTracker* zcwcTracker = pwallet->zcwcTracker.get();
    CMintMeta meta;
    if (!zcwcTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zcwcTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a CWC stake is the outpoint
