...
#include "ui_zcwccontroldialog.h"
...
void ZPivControlDialog::updateList()
{
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        {
            LOCK(pwalletMain->zcwcTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zcwcTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zCWC.";
            else if (!mint.isSeedCorrect)
                strReason = "The zCWC seed used to mint this zCWC is not the same as currently hold in the wallet";
            else
