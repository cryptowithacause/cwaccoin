...
/**
 * cryptowithacause RPC command dispatcher.
 */
class CRPCTable
{
private:
...
extern UniValue DoZcwcSpend(const CAmount nAmount, bool fMintChange, bool fMinimizeChange, vector<CZerocoinMint>& vMintsSelected, std::string address_str, bool isPublicSpend = true);
...
extern UniValue setzcwcseed(const UniValue& params, bool fHelp);
extern UniValue getzcwcseed(const UniValue& params, bool fHelp);
extern UniValue generatemintlist(const UniValue& params, bool fHelp);
extern UniValue searchdzcwc(const UniValue& params, bool fHelp);
extern UniValue dzcwcstate(const UniValue& params, bool fHelp);
